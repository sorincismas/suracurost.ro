...
class DatagridState extends StateAbstract {
    ...
    public function getStateParameters()
    {
        ...
        return (object)array(
            ...
            'per_page' => $this->getPerPage($data),
            'sorting' => !empty($data['sorting']) ? $data['sorting'] : null,
            'order_by' => !empty($data['order_by']) ? $data['order_by'] : null,
            'visible_columns' => !empty($data['visible_columns']) ? $data['visible_columns'] : null,
        );
    }

    protected function _getFieldsForEqualSearch() {
        $fieldsForEqualSearch = array_merge(
            array_keys($this->gCrud->getRelations1toMany()),
            array_keys($this->gCrud->getDynamicRelation1toN())
        );
    ...
    public function getSearchData() {
        ...
        // If the search data are coming from fields that we know we want only equal comparison (e.g. relation ids)
        // then we need to always add the equal sign at the beginning (so we will NOT search with LIKE %% )
