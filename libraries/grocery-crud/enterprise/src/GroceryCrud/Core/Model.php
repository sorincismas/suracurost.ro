...
ORDER BY KU.TABLE_NAME, KU.ORDINAL_POSITION;");
    ...
    public function validateMultiple($ids) {
        ...
        $select->from($this->getTableNameWithSchema($dbTableName));

        $primaryKeyField = $this->getPrimaryKeyField();
        $where = new Where();
        $where->in("$dbTableName.$primaryKeyField", $ids);
        $select->where($where);
    ...
    public function joinStatements($select)
    {
        foreach ($this->_relation_1_n as $relation) {
            ...
            // We are joining the tables for the following 3 scenarios:
            // 1. When optimize SQL queries is set to off
            // 2. When we have an order by field
            // 3. The relation has a where statement
            if (
                $this->_optimizeSqlQueries === false ||
                $this->orderBy === $relation->fieldName ||
                $relation->where !== null
            ) {
                $this->_setJoinForRelation($select, $relation);
            }
        }
    ...
    protected function _setJoinForRelation($select, $relation) {
    ...
    public function getRelationNtoNData($fieldInfo, $primaryKeyValue)
    {
        ...
        foreach ($results as $row) {
            $relationIds[] = (string)$row->{$fieldInfo->primaryKeyToReferrerTable};
        }
